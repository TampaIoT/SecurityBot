...
Compass.prototype.getRawValues = function (callback) {
    ...
    try {
        self.i2c.readI2cBlock(HMC5883L_ADDRESS, 3, BUF_LEN, buf, i2cCallback);
    } catch (ex) {
    ...
    function i2cCallback (err) {
        ...
        if (err) {
            ...
            callback(null, {
                x: (convert(0) + self.cal.offset.x) * self.cal.scale.x,
                y: (convert(2) + self.cal.offset.y) * self.cal.scale.y,
                z: (convert(4) + self.cal.offset.z) * self.cal.scale.z
            });
        }
        callback = null;
    }
	...
	console.log(offset, buf[offset], buf[offset + 1]);
        if (val === -4096) {
            return null;
        }
